...
Chúng ta định nghĩa \textbf{regularization (chuẩn hoá)} là ``bất kỳ sự điều chỉnh nào mà ta thực hiện đối với một thuật toán học nhằm giảm lỗi tổng quát hóa (generalization error) nhưng không làm giảm lỗi trên tập huấn luyện (training error).'' \cite{Goodfellow2016}
...
Trong một số tình huống, các hình phạt và ràng buộc là cần thiết để biến một bài toán chưa xác định (underdetermined) thành bài toán xác định. Ngoài ra, một số dạng regularization khác, được gọi là \textbf{phương pháp tổ hợp (ensemble methods)}, kết hợp nhiều giả thuyết khác nhau cùng giải thích dữ liệu huấn luyện.\cite{Bishop2006}
...
\]
với \(\alpha > 0\) là siêu tham số điều khiển cường độ chính quy hóa. \cite{Goodfellow2016}\cite{Bishop2006}
...
Tuy nhiên, sai số trên một tập dữ liệu độc lập (gọi là tập kiểm chứng -- validation set) thường chỉ giảm trong giai đoạn đầu, sau đó bắt đầu tăng lên khi mô hình rơi vào hiện tượng overfitting. Do đó, quá trình huấn luyện nên được dừng lại tại thời điểm mà sai số trên tập kiểm chứng đạt giá trị nhỏ nhất.
\cite{Ying2019}\cite{Bishop2006}
\begin{figure}[h]
\centering
...
Tuy nhiên, việc gia tăng kích thước dữ liệu cũng làm tăng thời gian huấn luyện. Bên cạnh đó, việc thu thập và gán nhãn dữ liệu mới thường tốn kém và đòi hỏi nhiều công sức. Vì vậy, thay vì thu thập dữ liệu hoàn toàn mới, người ta thường tận dụng dữ liệu hiện có để tạo ra các mẫu mới.
\cite{Ying2019}\cite{Goodfellow2016}
...
\cite{Bishop2006}\cite{Goodfellow2016}
...
Bên cạnh đó, kiểm chứng chéo (cross-validation) là một mở rộng của phương pháp trên, trong đó dữ liệu được chia thành nhiều phần và luân phiên sử dụng các phần này cho huấn luyện và kiểm định. Cách tiếp cận này giúp tận dụng tối đa dữ liệu và cung cấp ước lượng ổn định hơn về hiệu năng của mô hình.
\cite{AbuMostafa2012}
...
Trong trường hợp dữ liệu đặc biệt khan hiếm, có thể sử dụng biến thể \textbf{Leave-One-Out Cross-Validation (LOOCV)}, tương ứng với $S = N$ (với $N$ là số lượng mẫu). Khi đó, mỗi lần lặp chỉ giữ lại đúng một mẫu để kiểm định và sử dụng toàn bộ phần còn lại để huấn luyện.
...
Do đó, một hướng tiếp cận lý tưởng là xây dựng các tiêu chí đánh giá chỉ dựa trên dữ liệu huấn luyện, cho phép so sánh nhiều mô hình và siêu tham số trong một lần huấn luyện, đồng thời hạn chế sai lệch do overfitting.
\cite{Bishop2006}\cite{Hastie2009}
...
Trong trường hợp không thể mở rộng dữ liệu, việc cải thiện hiệu năng mô hình sẽ phụ thuộc vào việc phát triển các thuật toán học tốt hơn, đây thường là hướng nghiên cứu chuyên sâu hơn là áp dụng thực tiễn.
\cite{Bishop2006}\cite{Goodfellow2016}
\begin{figure}[h]
\centering
